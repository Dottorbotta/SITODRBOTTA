...
## Accessibilità con axe-core

Il workflow **Website accessibility** controlla tutte le pagine della sitemap a 1440 e 390 px, più il menu mobile aperto. Usa axe-core tramite Playwright/Chromium, con dipendenze e lockfile isolati in `.ci/accessibility`, senza cambiare React/Vinext. Controlla le regole automatiche WCAG A/AA 2.0/2.1/2.2 e best practice; verifica inoltre da tastiera il link «Salta al contenuto» e apertura, primo link e chiusura del menu mobile sulla homepage.

Le violazioni e gli errori di scansione fanno fallire il job: non sono nascoste da eccezioni. I report `accessibility-reports-*` includono riepilogo Markdown, dettagli JSON con selettori HTML e risultati incompleti da verificare manualmente, conservati per 14 giorni. Un esito verde non certifica l'accessibilità: restano necessarie verifiche con screen reader, zoom e utenti reali. Per riprodurre, avviare la build sulla porta 4173, installare il tooling con `npm ci --prefix .ci/accessibility` e Chromium con Playwright, eseguire `python3 scripts/collect-links.py`, quindi `node .ci/accessibility/audit.mjs`.
