...
import manifest from '../data/image-sources.json';

type Entry = { width: number; height: number; prefix: string; widths: number[] };
...
const srcSet = (entry: Entry, format: 'webp' | 'avif') => entry.widths.map(width => `${entry.prefix}-${width}.${format} ${width}w`).join(', ');
