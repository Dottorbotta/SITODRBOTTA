...
class AuditTests(unittest.TestCase):
    ...
    def test_decorative_empty_alt_and_unreviewed_are_valid(self):
        p,r=self.fixture();self.assertEqual([f for f in m.audit(p,r)[0] if f['priority']=='P1'],[])
    def test_missing_alt_is_not_decorative(self):
        p,r=self.fixture();p[0]['html']=p[0]['html'].replace('alt=""','');r[0].pop('imageAlt')
        findings=m.audit(p,r)[0]
        self.assertEqual({f['rule'] for f in findings if f['priority']=='P1'},{'missing-alt'})
        self.assertIn('undocumented-cover-alt',{f['rule'] for f in findings if f['priority']=='P2'})
    def test_broken_related_and_unsupported_review_fail(self):
        ...
        self.assertEqual({f['rule'] for f in m.audit(p,r)[0] if f['priority']=='P1'},{'missing-related','clinical-review-date','clinical-review-author'})
